function allData = plotEarlyLateCont(allData, varargin)
    % Function to analyze contrast performance in early vs late trials
    % and display the corresponding dopamine traces
    %
    % Input: 
    %   allData - structure containing processed photometry data
    %   varargin - optional parameters:
    %     'Contrasts' - array of contrast values to analyze (e.g., [10 20 50])
    %                   If not provided, all available contrasts are used
    % 
    ...
    if nargin > 1 
        for i = 1:2:length(varargin)
            if strcmpi(varargin{i}, 'Contrasts')
                contrasts_to_include = varargin{i+1};
                if ~isnumeric(contrasts_to_include)
                    error('The ''Contrasts'' parameter must be a numeric array of contrast values');
                end
            end
        end
    end
    ...
    for fileIdx = 1:length(allData)
        try
            ...
            % Filter contrasts if specific ones were requested
            if ~isempty(contrasts_to_include)
                % Find which contrasts match the requested ones
                [~, keepIdx, ~] = intersect(contrasts, contrasts_to_include);
                
                if isempty(keepIdx)
                    warning('None of the requested contrasts (%s) were found in dataset %d. Using all available contrasts.', ...
                        mat2str(contrasts_to_include), fileIdx);
                else
                    % Keep only the requested contrasts
                    contrasts = contrasts(keepIdx);
                    hitContrastFields = hitContrastFields(keepIdx);
                    missContrastFields = missContrastFields(keepIdx);
                    fprintf('Analyzing only contrasts: %s for dataset %d\n', ...
                        strjoin(arrayfun(@(x) num2str(x), contrasts, 'UniformOutput', false), ', '), fileIdx);
                end
            end
            ...
            % Create title, include contrast info if filtered
            if ~isempty(contrasts_to_include)
                contrastStr = sprintf(' (Contrasts: %s)', strjoin(arrayfun(@num2str, sortedContrasts, 'UniformOutput', false), ', '));
            else
                contrastStr = '';
            end
            sgtitle(sprintf('Session %d (%s) - Early vs Late Trial Performance%s', fileIdx, dateStr, contrastStr), 'FontSize', 16);
            ...
            % Filter DA contrasts if specific ones were requested
            if ~isempty(contrasts_to_include)
                [~, keepIdx, ~] = intersect(contrastNums, contrasts_to_include);
                if ~isempty(keepIdx)
                    contrastNums = contrastNums(keepIdx);
                    hitFields = hitFields(keepIdx);
                end
            end
            ...
            % Plot early traces - Inline instead of function call
            plotEarlyLateTraces(allData, fileIdx, sortedDAFields, sortedDAContrasts, 'early', medianTime, baseColors);
            ...
            % Plot late traces - Inline instead of function call
            plotEarlyLateTraces(allData, fileIdx, sortedDAFields, sortedDAContrasts, 'late', medianTime, baseColors);
...
% Helper function to plot dopamine traces (nested within the file)
function plotEarlyLateTraces(allData, fileIdx, sortedFields, sortedContrasts, timePeriod, medianTime, colors)
